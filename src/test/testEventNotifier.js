...
const formatEventMessage = (event) => {
  ...
  return {
    text: `<b>🎉 New Superteam Ireland Event!</b>\n\n` +
          `<b><a href="${event.eventUrl}">${event.eventName}</a></b>\n\n` +
          `📅 <b>Date:</b> ${eventDate}\n` +
          `📍 <b>Location:</b> ${event.eventLocation}\n\n` +
          `🔗 <a href="${event.eventUrl}">Click here to RSVP</a>\n`,
    options: {
      parse_mode: 'HTML',
...
const testEventNotifications = async () => {
  let bot;
  try {
    console.log('🧪 Event Notification System Test');
    console.log('================================');
    ...
    for (const event of events) {
      ...
      const { text, options } = formatEventMessage(event);
